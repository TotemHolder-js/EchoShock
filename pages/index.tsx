...
export default function Home({ games, latestEcho, pinnedEcho }: HomeProps) {
  return (
    <Layout title='EchoShock - Level the Playing Field'>
      {pinnedEcho && (
        <section className='mb-16'>
          <h2 className='text-2xl font-bold mb-4 hover-glow'>Featured Echo</h2>
...
export const getStaticProps: GetStaticProps = async () => {
  ...
  if (gamesError || echoError || pinnedError) {
    console.error(
      'Error fetching home data:',
      gamesError || echoError || pinnedError
    )
    return {
      props: {
        games: [],
